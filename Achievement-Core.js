...
const PLUGINS_INFO = {
    name: "Achievement", version: "v2.0.2",
};
        ...
        zh_CN: {
            destroyBlock: {
                enable: true, name: "挖掘成就", ui: "textures/ui/icon_iron_pickaxe", details: {
                    ...
                    "minecraft:diamond_ore": new Achievement("谁不喜欢钻石呢?", "首次挖掘钻石矿"),
        ...
        zh_CN: {
            killer: {
                enable: true, name: "击杀成就", ui: "textures/ui/promo_creeper", details: {
                    ...
                    "minecraft:sheep": new Achievement("谁会杀害温顺又可爱的绵羊呢？", "首次击杀绵羊"),
...
class ScoreChange {
    ...
    static async scoreMoneyImpl(pl, num, name, disName) {
        if (Runtime.config.reward.economy.type === Constant.moneyType.score && name === Runtime.config.reward.economy.score) {
            let type = ScoreChange.ECONOMY_TYPE;//此为默认配置好的经济计分板项成就
...
class LLmoneyAdd {
    ...
    static EVENT = "onMoneyAdd";

    static EventImplList = ["defaultImpl"];

    static process(xuid, money) {
        if (Utils.hasNullOrUndefined(xuid, money)) return;
        LogUtils.debug(`参数列表: `, ...arguments);
        LogUtils.debug(`事件: ${LLmoneyAdd.EVENT} 名称:玩家LLMONEY金额增加 XUID:${xuid} 金额:${money}`);
        EventProcessor.eventImplsProcess(LLmoneyAdd, [xuid], LLmoneyAdd.EventImplList).catch(err => {
            LogUtils.error(`事件${LLmoneyAdd.EVENT}: `, err);
        });
    }

    static defaultImpl(xuid) {
        return LLmoneyAdd.llMoneyImpl(xuid);
    }

    static async llMoneyImpl(xuid) {
        let pl = mc.getPlayer(xuid);
        let type = ScoreChange.ECONOMY_TYPE;
        let num = pl.getMoney();

        //配置文件是否是llmoney
        if (Runtime.config.reward.economy.type !== Constant.moneyType.llmoney) return;
        return NumberChange.defaultImpl(pl, type, num, true);
    }
}

class LLmoneyReduce {

    static EVENT = "onMoneyReduce";
    ...
    static process(xuid, money) {
        if (Utils.hasNullOrUndefined(xuid, money)) return;
        LogUtils.debug(`参数列表: `, ...arguments);
        LogUtils.debug(`事件: ${LLmoneyAdd.EVENT} 名称:玩家LLMONEY金额减少 XUID:${xuid} 金额:${money}`);
        EventProcessor.eventImplsProcess(LLmoneyReduce, [xuid], LLmoneyReduce.EventImplList).catch(err => {
            LogUtils.error(`事件${LLmoneyReduce.EVENT}: `, err);
        });
    }
...
class LLmoneySet {
    ...
    static process(xuid, money) {
        if (Utils.hasNullOrUndefined(xuid, money)) return;
        LogUtils.debug(`参数列表: `, ...arguments);
        LogUtils.debug(`事件: ${LLmoneyAdd.EVENT} 名称:玩家LLMONEY金额设置 XUID:${xuid} 金额:${money}`);
        EventProcessor.eventImplsProcess(LLmoneySet, [xuid], LLmoneySet.EventImplList).catch(err => {
            LogUtils.error(`事件${LLmoneySet.EVENT}: `, err);
        });
    }
...
class EventProcessor {
     ...
     * @type {Array}
     */
    static EVENT_PROCESSOR_LIST = [Join, Left, ChangeDim, Destroy, Place, AttackEntity, MobHurt, MobDie, PlDie, ScoreChange, LLmoneyAdd, LLmoneyReduce, LLmoneySet, ConsumeTotem, InventoryChange, UseBucketTake, DropItem, Eat, ArmorSet, BedEnter, ProjectileHitEntity, ProjectileCreated, AfterFinished];
